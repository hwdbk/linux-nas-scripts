...
- the `cleanup_attrs` script is a handy tool to view, review and prune (what I consider totally useless and obnoxious) attribute data. It uses the `xattrs.lst` exclusion list of extended attributes that I _do_ find useful. The idea is that you first list the attributes that are present in/under a path (if you have a chance, try it out and be amazed - with `-l` it does not modify the file system), and if you find attributes that are useful to you, just add them to the `xattrs.lst` to exclude them. The end result is a true spring cleaning: with all the garbage gone, directory browsing from MacOS resumes at lightning speed.
