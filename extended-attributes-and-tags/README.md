...
- the `cleanup_attrs` script is a handy tool to view, review and prune (what I consider totally useless and obnoxious) attribute data. It uses the `xattrs.lst` exclusion list of extended attributes that I _do_ find useful. The idea is that you first review the attributes found in/under a path using the `-l` (list) option, and if you find attributes that are useful to you, just add them to the `xattrs.lst` to exclude them in subsequent runs. If not, you can remove them from the files and directories with `-x` (`-n` (dryrun) will show first what would be done) and `-v` (verbose) for detailed logging. The end result is a true spring cleaning: with all the garbage gone, directory browsing from MacOS resumes at lightning speed.
